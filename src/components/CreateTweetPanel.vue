<template>
    <form action="" class="create-tweet-panel" @submit.prevent="createNewTweet" :class="{'exceeded': newTweetCharacterCount > 180 }">
        <label for="newTweet"><strong>New Tweet</strong>({{newTweetCharacterCount}}/180)</label>
        <textarea name="" id="newTweet" cols="30" rows="4" v-model="state.newTweetContent"></textarea>
...
export default {
    ...
    setup (props, context) {
        ...
        function createNewTweet() {
            if(state.newTweetContent && state.selectedTweetType !== 'draft' && state.newTweetContent.length <= 180) {
